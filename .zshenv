...
alias gmm='f() { git commit --amend -m "$*" }; f'
alias glod='f() { git --no-pager log --graph --pretty="%Cred%h%Creset%C(auto)%d%Creset %s %Cgreen(%ad) %C(bold blue)%an <%ae>%Creset" -n "${1:-7}"; }; f'
...
alias cnt='f() { cargo nextest run "$*" }; f'
