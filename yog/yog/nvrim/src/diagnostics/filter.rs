...
pub fn filter(lsp_diags: Vec<Dictionary>) -> Vec<Dictionary> {
    ...
    if BufferFilter::new()
        ...
        .inspect_err(|error| {
            crate::oxi_ext::notify_error(&format!("error filtering by buffer {buf_path}, error {error:#?}"));
        })
        .is_ok_and(identity)
    {
        return vec![];
    ...
    let Ok(filters) = DiagnosticsFilters::all(&lsp_diags).inspect_err(|error| {
        crate::oxi_ext::notify_error(&format!("cannot get diangnostics filters, error {error:#?}"));
    }) else {
        return vec![];
    };
