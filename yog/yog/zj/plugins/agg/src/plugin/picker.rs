...
pub fn update_panes(state: &mut PickerState, manifest: &PaneManifest) -> bool {
    let events = crate::plugin::picker::events_from::pane_update::derive(
        state,
        manifest,
...
pub fn update_command(state: &mut PickerState, pane_id: PaneId, command: &[String], is_foreground: bool) -> bool {
    if !is_foreground {
        return false;
    }
...
pub fn pipe(state: &mut PickerState, pipe_message: &PipeMessage) -> bool {
    ...
    if is_state_snapshot {
        let Ok(snapshot) =
            StateSnapshotPayload::try_from(pipe_message).inspect_err(|error| eprintln!("agg picker: {error}"))
        else {
            return false;
        };
...
pub fn update_run_command_result(
    ...
    match context.get(CONTEXT_KIND).map(String::as_str) {
        Some(CONTEXT_AGS_SESSIONS) => {
            if exit_code != Some(0) {
                eprintln!(
                    "agg picker: ags list --json failed: {}",
