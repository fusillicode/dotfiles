    ...
    fn apply(&mut self, event: &Event) {
        match event {
            ...
            Event::AgentIdle { pane_id, agent } => {
                ...
                if let Some(current_tab) = self.current_tab.as_mut() {
                    let phase = crate::plugin::tbar::current_tab::idle_phase_for_pane(
                        current_tab,
...
mod tests {
    ...
    fn test_agent_busy_clears_nudge_dedupe() {
        ...
        let idle_events = crate::plugin::tbar::events_from::agent::derive(
            &state,
            &AgentEventPayload {
                pane_id: 42,
                agent: Agent::Codex,
        ...
        assert2::assert!(Nudge::new(current_tab, &state.all_tabs, &state.home_dir, 42).is_some());
        state.mark_nudged(42);

        let busy_events = crate::plugin::tbar::events_from::agent::derive(
            &state,
            &AgentEventPayload {
                pane_id: 42,
                agent: Agent::Codex,
        ...
        assert2::assert!(state.nudged_pane_ids.is_empty());
