...
pub fn load(state: &mut TabBarState, home_dir: PathBuf) {
    state.plugin_id = zellij_tile::prelude::get_plugin_ids().plugin_id;
    state.home_dir = home_dir;
}

pub fn update_permission_granted(state: &mut TabBarState) -> bool {
...
pub fn render(state: &TabBarState, rows: usize, cols: usize, buf: &mut String) {
    crate::plugin::tab_bar::ui::render_frame(&state.frame, rows, cols, buf);
}

pub fn update_tabs(state: &mut TabBarState, mut tabs: Vec<TabInfo>) -> bool {
    ...
    let events = crate::plugin::tab_bar::events_from::tab_update::derive(state, &mut tabs, landing_focus);
...
pub fn update_panes(state: &mut TabBarState, manifest: &PaneManifest) -> bool {
    let events = crate::plugin::tab_bar::events_from::pane_update::derive(state, manifest, |pane_id| {
        zellij_tile::prelude::get_pane_cwd(PaneId::Terminal(pane_id)).ok()
    });
    apply_and_handle_events(state, &events)
}

pub fn update_pane_closed(state: &mut TabBarState, pane_id: u32) -> bool {
    let events = crate::plugin::tab_bar::events_from::pane_close::derive(state, pane_id);
    apply_and_handle_events(state, &events)
}

pub fn update_cwd(state: &mut TabBarState, pane_id: u32, cwd: PathBuf) -> bool {
    let events = crate::plugin::tab_bar::events_from::cwd::derive(state, pane_id, cwd);
    apply_and_handle_events(state, &events)
}
...
pub fn update_run_command_result(
    ...
    let events = crate::plugin::tab_bar::events_from::run_command::derive(state, &requested_cwd, exit_code, stdout);
    apply_and_handle_events(state, &events)
}

pub fn update_mouse_left_click(state: &TabBarState, row: isize, last_cols: usize) -> bool {
    let Ok(row) = usize::try_from(row) else {
        return false;
    };
    let content_w = last_cols.saturating_sub(1);
    if let Some(tab_idx) = crate::plugin::tab_bar::ui::tab_index_at_row(&state.frame, row, content_w)
...
pub fn pipe(state: &mut TabBarState, pipe_message: &PipeMessage) -> bool {
    ...
    match event {
        ...
        PipeEvent::ActiveTab { active_tab_id } => {
            ...
            let events = crate::plugin::tab_bar::events_from::active_tab::derive(state, active_tab_id, landing_focus);
            let frame_changed = state.apply_all(&events);
            handle_events(state, &events);
            frame_changed || !events.is_empty()
        }
        PipeEvent::StateSnapshot {
            ...
            let events = crate::plugin::tab_bar::events_from::snapshot::derive(state, source_plugin_id, &snapshot);
            let frame_changed = state.apply_all(&events);
            frame_changed || !events.is_empty()
        }
        PipeEvent::Agent(agent_event) => {
            let events = crate::plugin::tab_bar::events_from::agent::derive(state, &agent_event);
            let frame_changed = state.apply_all(&events);
            handle_events(state, &events);
            frame_changed || !events.is_empty()
        }
...
fn apply_and_handle_events(state: &mut TabBarState, events: &[Event]) -> bool {
    let frame_changed = state.apply_all(events);
    handle_events(state, events);
    frame_changed || !events.is_empty()
}

fn handle_events(state: &mut TabBarState, events: &[Event]) {
    for event in events {
        match event {
...
pub mod test_support {
    ...
    use crate::plugin::tab_bar::current_tab::AgentPaneState;
    ...
    pub fn apply_pane_update(state: &mut TabBarState, manifest: &PaneManifest) -> Vec<Event> {
        let events = crate::plugin::tab_bar::events_from::pane_update::derive(state, manifest, noop_pane_cwd);
...
mod tests {
    ...
    fn test_resolve_active_tab_landing_focus_falls_back_to_focused_tracked_pane_when_host_focus_missing() {
        ...
        let landing_focus = crate::plugin::tab_bar::resolve_active_tab_landing_focus_with(
            20,
            &tabs,
            Some(&current_tab),
            || None,
