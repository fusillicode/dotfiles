...
impl PickerState {
    ...
    pub fn handle_key(&mut self, key: &KeyWithModifier) -> PickerAction {
        match key.bare_key {
            ...
            | BareKey::Down
            | BareKey::Up
            | BareKey::Char(_) => PickerAction::None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_agent_events_follow_tab_bar_marker_transitions() {
        let mut state = PickerState::default();
        let _ = state.update_tabs(vec![TabInfo {
            active: true,
            position: 0,
    ...
    fn test_handle_key_updates_query_backspace_esc_empty_and_enter() {
        ...
        assert_eq!(state.handle_key(&key(BareKey::Char('c'))), PickerAction::Redraw);
        assert_eq!(state.handle_key(&key(BareKey::Backspace)), PickerAction::Redraw);
        assert_eq!(state.query, "");
        assert_eq!(state.handle_key(&key(BareKey::Esc)), PickerAction::Close);
        assert_eq!(state.handle_key(&key(BareKey::Enter)), PickerAction::Focus(42));
        assert_eq!(state.handle_key(&key(BareKey::Char('x'))), PickerAction::Redraw);
        assert_eq!(state.handle_key(&key(BareKey::Enter)), PickerAction::None);
    }
    ...
    fn test_handle_key_selection_loop_is_noop_for_empty_or_single_result() {
        let ctrl_n = KeyWithModifier::new_with_modifiers(BareKey::Char('n'), BTreeSet::from([KeyModifier::Ctrl]));
        let ctrl_p = KeyWithModifier::new_with_modifiers(BareKey::Char('p'), BTreeSet::from([KeyModifier::Ctrl]));
        let mut empty_state = PickerState::default();
        let mut single_state = PickerState {
            pane_entries: vec![PaneEntry::new(0, 42, None, vec![String::from("cargo")], None)],
            ..Default::default()
        };

        assert_eq!(empty_state.handle_key(&ctrl_n), PickerAction::None);
        assert_eq!(empty_state.handle_key(&ctrl_p), PickerAction::None);
        assert_eq!(single_state.handle_key(&ctrl_n), PickerAction::None);
        assert_eq!(single_state.handle_key(&ctrl_p), PickerAction::None);
