...
impl PaneEntry {
    ...
    pub fn apply_command(&mut self, command_args: Vec<String>) -> bool {
        ...
        self.session_id = crate::plugin::ppick::entry::resume_session_id_from_command_args(&self.command_args);
...
fn resume_session_id_from_command_args(args: &[String]) -> Option<String> {
    ...
    match command {
        "codex" => crate::plugin::ppick::entry::codex_resume_id(args),
        "claude" | "cursor-agent" => crate::plugin::ppick::entry::resume_flag_id(args),
        _ => None,
    }
}
