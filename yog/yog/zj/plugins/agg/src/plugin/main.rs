...
use crate::plugin::events::PipeEvent;
use crate::plugin::events::PipeEventError;
    ...
    GetPaneInfo: FnMut(PaneId) -> Option<PaneInfo>,
{
    ...
    if let Some((focused_tab_position, focused_pane_id)) = get_focused_pane_info()
        ...
        && let Some(focused_pane) = crate::plugin::state::pane::focused_pane_from_pane_info(&pane)
    {
    ...
    current_tab.pane_ids.iter().find_map(|pane_id| {
        ...
        crate::plugin::state::pane::focused_pane_from_pane_info(&pane)
    })
}
