...
mod file_target_tests {
    ...
    #[case("bar/baz-\n    foo.yaml:222", Some(("bar/baz-foo.yaml", 222, 1)))]
    ...
    fn test_file_target_reconstructed_from_lines_returns_ambiguous_for_multiple_targets() {
        let lines = [
            "/tmp/first/foo.yaml:222".to_owned(),
            "/tmp/second/foo.yaml:222".to_owned(),
        ];

        pretty_assertions::assert_eq!(
            FileTargetReconstructed::from_lines(&lines, "foo.yaml:222"),
