...
fn install_layout() -> rootcause::Result<()> {
    let source = assets_dir()?.join("zellij").join(LAYOUT_FILENAME);
    zj::install_layout_file(&source, LAYOUT_FILENAME)
}

fn assets_dir() -> rootcause::Result<PathBuf> {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .ok_or_else(|| rootcause::report!("zj manifest dir has no parent"))?
        .join("agg")
        .join("assets");
    Ok(dir)
}
...
fn install_opencode_plugin() -> rootcause::Result<()> {
    ...
    let template = assets_dir()?.join("opencode").join(OPENCODE_TEMPLATE_FILENAME);
